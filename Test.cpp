...
TEST_CASE("Sniper VS Sniper") { //good
    WarGame::Board board(8,8);
            CHECK(!board.has_soldiers(1)); // no soldiers added
    board[{0,1}] = new Sniper(1); //add new solider to player 1
            CHECK(board.has_soldiers(1)); // should return true

            CHECK(!board.has_soldiers(2)); // no soldiers added to player 2
    board[{7,1}] = new Sniper(2); // add new soldier to player 2
            CHECK(board.has_soldiers(2)); // shold return true

    board.move(1,{0,1},WarGame::Board::MoveDIR::Up); // move soldier
            CHECK(board.has_soldiers(2));
            CHECK(board.has_soldiers(1));
    CHECK_THROWS(board.move(1,{0,1},WarGame::Board::MoveDIR::Up)); //the soldier is no longer in this location
    board.move(1,{1,1},WarGame::Board::MoveDIR::Up); // the right location
            CHECK(!board.has_soldiers(2)); // player 2's soldier is dead
            CHECK(board.has_soldiers(1));
}
...
TEST_CASE("Foot vs Foot") {
    ...
    CHECK_THROWS(board.move(1,{7,1},WarGame::Board::MoveDIR::Up)); // the soldier belong to the other player
    CHECK_THROWS(board.move(2,{7,1},WarGame::Board::MoveDIR::Up));//out of bounds
    ...
    CHECK_THROWS(board.move(1,{3,1},WarGame::Board::MoveDIR::Up));//there is another soldier in the location
    CHECK_THROWS(board.move(2,{4,1},WarGame::Board::MoveDIR::Down)); //same
            CHECK(board.has_soldiers(2));
            CHECK(board.has_soldiers(1));
    ...
    board.move(2,{4,1},WarGame::Board::MoveDIR::Down); //soldier 1 - 0
            CHECK(board.has_soldiers(2));
            CHECK(!board.has_soldiers(1)); //lose
...
TEST_CASE("3 VS 3") {
    ...
    std::cout << board[{7,2}]->getPoints() << std::endl;
